...
class TestSearchResults(unittest.TestCase):
    ...
    def test_contains(self):
        ...
        results.report(1, {'lr': 0.1}, 0.5)
        self.assertTrue(1 in results)

    def test_new_results(self):
        results = SearchResults(experiment=logreg.experiment)
        df = results.new_results()
        self.assertEqual(['ix', 'optimization.lr', 'val_loss'],
                         list(df.columns))

    def test_report(self):
        results = SearchResults(experiment=logreg.experiment)
        results.report(1, {'optimization.lr': 0.1}, 0.5)
        expected = [{'ix': 1, 'optimization.lr': 0.1, 'val_loss': 0.5}]
    ...
    def test_best(self):
        results = SearchResults(experiment=logreg.experiment)
        results.report(1, {'optimization.lr': 0.1}, 0.5)
        results.report(2, {'optimization.lr': 0.3}, 0.7)
        results.report(2, {'optimization.lr': 0.3}, 0.5)
        expected = [{'ix': 1, 'optimization.lr': 0.1}]
